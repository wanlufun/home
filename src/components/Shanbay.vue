...
const getHitokotoData = async () => {
  try {
    const result = await getShanbay();
    hitokotoData.text = result.content;
    hitokotoData.from = result.author;
  } catch (error) {
    ElMessage({
      message: "一言获取失败",
      icon: h(Error, {
    ...
    hitokotoData.text = "Quietly come, Quietly go, Quietly strive, Quietly reap.";
    hitokotoData.from = "Lu Wan";
  }
};
